...
function checkALPHA5(ARGV,CVALUE)
{
	document.getElementById(ARGV).value=document.getElementById(ARGV).value.replace(/^\s+/, '').replace(/\s+$/, '')
	document.getElementById(ARGV).value=document.getElementById(ARGV).value.toUpperCase();
	nm=document.getElementById(ARGV).value;
	...
	if(flag==1)
	{		
		alert(CVALUE+ " Only Alphabates allowed \nFill the Entry");
		document.getElementById(ARGV).value="";
		document.getElementById(ARGV).style.borderColor="red";
		return false;
	}
...
function checkALPHA4(ARGV,CVALUE,HDIV)
{
	...
	for(i=0;i<ln;i++)
	{
		...
		if((nm.charCodeAt(i)>=48 && nm.charCodeAt(i)<=57) || (nm.charCodeAt(i)>=65 && nm.charCodeAt(i)<=90) || (nm.charCodeAt(i)>=97 && nm.charCodeAt(i)<=122) || nm.charCodeAt(i)==32 || nm.charCodeAt(i)==44 || nm.charCodeAt(i)==46 ||nm.charCodeAt(i)==38)
		{						  
			flag = 0;
		}
		else
...
function checkStringNoSpace2(ARGV,MSG)
{
	nm=document.getElementById(ARGV).value;
	nm=nm.toUpperCase();
	document.getElementById(ARGV).value=nm;
	...
	if(flag==1)
	{		
		alert("Space not allowed in"+ MSG +"\nPlease  Fill "+ MSG +"");
		document.getElementById(ARGV).value="";
		document.getElementById(ARGV).style.borderColor="red";
		return false;
	}
...
function checkMobile(ARGV,CVALUE)
{
	...
	vl=parseInt(vl)
	if(vl<7000000001 || vl>9999999999)
	{
...
//	nm=nm.toUpperCase();
	...
	if(flag==1)
	{		
		alert(" Space not allowed  in "+CVALUE+" \nPlease  Fill "+CVALUE+"");
		document.getElementById(ARGV).value="";
		document.getElementById(ARGV).style.borderColor="red";
		return false;
	}
...
//					alert(dy +"\n"+maxD);
	...
	function checkSelect(ARGV,CVALUE) 
	{
	...
	function checkSelect2(ARGV,CVALUE,HDIV) 
	{
	   	var csel = document.getElementById(ARGV).value;
	   	if(csel=="999") 
		{
	...
	function MouseEfect(BANAM,ARGV)
		{
			if(ARGV==1)
			{
				document.getElementById(BANAM).style.background="#003366";
			}
			if(ARGV==2)
			{
				document.getElementById(BANAM).style.background="#003366";
